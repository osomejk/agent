...
import { ChevronDown, ChevronUp, Bug, Trash2, RefreshCw, AlertTriangle } from "lucide-react"
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  // Assuming forceResetCart and addToLog are defined elsewhere and accessible
                  // const success = await forceResetCart(token);
                  const success = true // Placeholder for actual implementation
                  const addToLog = (message: string) => console.log(message)

                  if (success) {
                    addToLog("Force reset successful")
                    // Clear local storage
                    localStorage.removeItem("cart")
                    localStorage.removeItem("cartQuantities")
                    loadDebugInfo()
                  } else {
                    addToLog("Force reset failed")
                  }
                } catch (e) {
                  console.error(e)
                }
